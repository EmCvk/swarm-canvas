...
import { CheckCircle2, Film, Loader2, Play, RefreshCw, Upload, XCircle } from 'lucide-react';
...
function replaceTokens(v:any, s:any, source:string):any {
  if (typeof v === 'string') return v.replaceAll('__PROMPT__',s.prompt).replaceAll('__NEGATIVE__',s.negative).replaceAll('__SEED__',String(s.seed)).replaceAll('__WIDTH__',String(s.width)).replaceAll('__HEIGHT__',String(s.height)).replaceAll('__FRAMES__',String(s.frames)).replaceAll('__FPS__',String(s.fps)).replaceAll('__DENOISE__',String(s.denoise)).replaceAll('__SOURCE_IMAGE__',source);
...
export const AnimationLab:React.FC=()=>{
  ...
  return <div className="h-full w-full min-h-0 bg-[#090a0d] text-zinc-100 flex flex-col">
